package MasteringJavaCollections;

import java.util.*;

public class LinkedHashSetAndTreeSet {
    static void main(String[] args) {
        List<Contact> phones = ContactData.getData("phone");
        List<Contact> emails = ContactData.getData("email");

        Comparator <Contact> mySort = Comparator.comparing(Contact::getName);
        NavigableSet<Contact> sorted = new TreeSet<>(mySort);
        sorted.addAll(phones);
        sorted.forEach(System.out::println);

        NavigableSet<String> justNames = new TreeSet<>();
        phones.forEach(c -> justNames.add(c.getName()));
        for (String justName : justNames) {
            System.out.println(justName);
        }

        NavigableSet<Contact> fullSet = new TreeSet<>(sorted);
        fullSet.addAll(emails);
        for (Contact contact : fullSet) {
            System.out.println(contact);
        }

        List<Contact> fullList = new ArrayList<>(phones);
        fullList.addAll(emails);
        fullList.sort(sorted.comparator());
        System.out.println("-".repeat(10));
        fullList.forEach(System.out::println);

        Contact min = Collections.min(fullSet, fullSet.comparator());
        Contact max = Collections.max(fullSet, fullSet.comparator());

        Contact first = fullSet.first();
        Contact last = fullSet.last();

        System.out.println(min.getName());
        System.out.println(max.getName());
        System.out.println(first.getName());
        System.out.println(last.getName());

        NavigableSet<Contact> copied = new TreeSet<>(fullSet);
        Contact pollFirst = copied.pollFirst();
        Contact pollLast = copied.pollLast();

        System.out.println(pollFirst);
        System.out.println(pollLast);
        System.out.println("-".repeat(10));
        copied.forEach(System.out::println);
        System.out.println("-".repeat(10));

        Contact daffy = new Contact("Daffy Duck");
        Contact daisy = new Contact("Daisy Duck");
        Contact snoopy = new Contact("Snoopy");
        Contact archie = new Contact("Archie");

        for (Contact c : List.of(daffy, daisy, first, archie)) {
            System.out.printf("floor(%s)=%s%n", c.getName(), fullSet.floor(c));
            System.out.printf("lower(%s)=%s%n", c.getName(), fullSet.lower(c));
        }
        System.out.println("------------------------------------");

        NavigableSet<Contact> descending = fullSet.descendingSet();
        descending.forEach(System.out::println);
        System.out.println("-".repeat(10));

        Contact last2 = descending.pollLast();
        System.out.println(last2);
        System.out.println("-".repeat(10));
        descending.forEach(System.out::println);
        System.out.println("-".repeat(10));

        Contact marion = new Contact("Maid Marion");
        var headSet = fullSet.headSet(marion, true);
        headSet.forEach(System.out::println);
        System.out.println("-----------------");

        var tailSet = fullSet.tailSet(marion, false);
        tailSet.forEach(System.out::println);
        System.out.println("-----------------");


        Contact linus = new Contact("Linus Van Pelt");
        var subSet = fullSet.subSet(linus,false, marion, true);
        subSet.forEach(System.out::println);


    }
}
