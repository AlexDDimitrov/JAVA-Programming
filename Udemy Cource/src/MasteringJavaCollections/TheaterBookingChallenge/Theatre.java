package MasteringJavaCollections.TheaterBookingChallenge;
...
public class Theatre{
    ...
    public Theatre(String theatreName, int rows, int totalSeats) {
        ...
        for (int i = 0; i < totalSeats; i++) {
            char rowC = (char) (i/seatsPerRow + (int) 'A');
            int seatsInRow = i % seatsPerRow + 1;
            seats.add(new Seat(rowC, seatsInRow));
        }
    }
    ...
    public void printMap() {
        ...
        int i = 0;
        for (Seat s : seats) {
            System.out.printf("%-10s%s", s.seatNum + (s.reserved ? "(\u25CF)" : ""),
                    ((i++ + 1) % seatsPerRow == 0) ? "\n" : "");
        }
